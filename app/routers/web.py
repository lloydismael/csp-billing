...
from app.services.ingestion import delete_upload, is_supported_upload_filename, mark_upload_completed, mark_upload_failed, process_upload_csv
...
async def dashboard(
    ...
    upload_cache_meta = {
        str(upload.id): {
            ...
            "completed_at": upload.completed_at.isoformat() if upload.completed_at else "",
            "cache_version": f"{upload.id}:{upload.row_count}:{upload.completed_at.isoformat() if upload.completed_at else ''}",
    ...
    return templates.TemplateResponse(
        "dashboard.html",
        {
            ...
            "upload_cache_meta_json": json.dumps(upload_cache_meta),
...
async def upload_file(
    ...
    if not is_supported_upload_filename(file.filename):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="File must be a CSV or XLSX")
